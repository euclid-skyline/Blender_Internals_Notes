...
| [`Blender_GPU_layer.md`](./Blender_GPU_layer.md)           | GPU abstraction layer        | Documents the GPU abstraction layer in `source/blender/gpu/`: backend selection (OpenGL/Vulkan/Metal), the `GPUBackend` abstract factory, the `ShaderCreateInfo` descriptor system, GPU batching (`VertBuf`, `IndexBuf`, `Batch`), immediate mode, and pipeline state management. |
...
11. [`Blender_GPU_layer.md`](./Blender_GPU_layer.md)

This path moves from **build/setup** → **startup flow** → **runtime context** → **operators and commands** → **WM loop and event/update systems** → **core data and evaluation layer** → **DNA data-definition and file format schema** → **RNA reflection and Python/UI/animation API** → **GPU abstraction layer and rendering pipeline**.
